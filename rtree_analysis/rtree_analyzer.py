...
def _create_bounding_box(coordinate, mile_range):
    center_point = Point(coordinate[0], coordinate[1])
    north_point = distance.distance(miles=mile_range).destination(center_point, 0)
    south_point = distance.distance(miles=mile_range).destination(center_point, 180)
    east_point = distance.distance(miles=mile_range).destination(center_point, 90)
    west_point = distance.distance(miles=mile_range).destination(center_point, 270)
...
def _get_scout_hubs_in_bounding_box(outpost_coordinate, mile_range, rtree):
...
def _hub_within_exact_outpost_radius(scout_hub_coordinate, outpost_coordinate, mile_range):
    points_distance = _distance_between(outpost_coordinate, scout_hub_coordinate)
    if points_distance <= mile_range:
...
def scan_outposts_range(year, outposts, scouts, mile_range: int):
    ...
    for i, (outpost_coordinate, outpost) in enumerate(outposts.items()):
        scout_hubs_in_bbox = _get_scout_hubs_in_bounding_box(outpost_coordinate, mile_range, rtree)
        ...
        for scout_hub in scout_hubs_in_bbox:
            ...
            distance_result = _hub_within_exact_outpost_radius(scout_hub_coordinate, outpost_coordinate, mile_range)
            if distance_result:
                scouts_in_range = scouts[scout_hub_coordinate]
                outpost.add_scouts(year=year,
